...
campusLabel / spaceLabel como texto libre. Equipo 1 (HU-03) aún no entrega el catálogo de Campus/Space en el backend, así que el modelo Report guarda el campus y la ubicación como texto en vez de una FK real. Cuando ellos entreguen su catálogo, hay que migrar estos dos campos a campus_id / space_id.
POST /reports/ es una versión mínima. El formulario completo de alta (HU-05/06) es de Equipo 1. Este POST solo existe para poder meter datos de prueba y que la consulta (HU-4) sea demostrable hoy. Avisen a Equipo 1 para no duplicar el endpoint cuando ellos conecten su formulario.
"Folio" = el id autoincremental del backend (un número, ej. 1, 2...). No es el mismo tipo de id que usa el mock del frontend ("rep-banos-sucios"). Por eso se creó ApiReport como tipo separado de Report en el frontend — no se tocó el modelo mock existente, así que no debería romper el resto de las pantallas (técnico, responsable, etc.) que siguen usando datos mock.
Cómo aplicarlo
Backend
Copia el contenido de backend/ dentro de tu clon de trabajo-colaborativo-backend, respetando las rutas (sobrescribe app/urls.py y alembic/env.py).
Levanta los contenedores si no están corriendo:
docker compose up -d
Aplica la migración:
docker compose exec api alembic upgrade head
Verifica en Swagger: http://localhost:8000/docs — deberías ver la sección Reports.
Frontend
Copia el contenido de frontend/ dentro de tu clon de trabajo-colaborativo-frontend, respetando las rutas (sobrescribe app.routes.ts, home/home.ts, home/home.html).
npm install si hace falta, luego ng serve.
Entra a /inicio/buscar (o dale clic a "Buscar por folio" desde la pantalla de Reportante).
Cómo probar (lo que pide el cronograma: "búsqueda por folio y visualización del estado inicial")
Regístrate / inicia sesión (endpoints de Equipo 1, ya existen: POST /auth/register, POST /auth/login) para obtener un access_token.
...
En el frontend, en /inicio/buscar, escribe 1 en el campo Folio y dale "Buscar" — debe mostrar el título, la ubicación y la línea de tiempo con el primer punto ("Creado") activo, que es justo el estado inicial "Reportada" que pide probar el cronograma.
